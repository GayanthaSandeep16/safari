...
export default function CreateSafariGroupPage() { // Renamed for clarity
  ...
  const updateGroupWithMembersAndPassengers = useMutation(api.mutations.updateGroupWithMembersAndPassengers);
  ...
  const handleAddMember = () => {
    ...
    if (members.length >= maxAdditionalMembers) {
      toast.error(`Maximum ${maxAdditionalMembers} additional members allowed for this group size.`);
      return;
    }

    if (!newMember.name || !newMember.country || newMember.age <= 0 || !newMember.phone || newMember.gender === '') {
      toast.error('Please fill all member details: Name, Country, Age, Gender, and Phone.');
      return;
    }
  ...
  const handleCreateSafariGroup = async () => {
    ...
    if (currentSize > maxCapacity) {
      toast.error(`Group size (${currentSize}) exceeds maximum capacity of ${maxCapacity}.`);
      return;
    }
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Set up a new safari adventure with your group.</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Provide the core information for your safari.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              <Input
                ...
                className="w-4 h-4"
              />
              <Label htmlFor="hasGuide">Include a professional guide (max 7 people total)</Label>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Input
                ...
                className="w-4 h-4"
              />
              <Label htmlFor="isShared">Allow others to join this group (make it public)</Label>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-6">
            ...
            {members.length > 0 && (
              <div className="mt-6 space-y-2">
                <h3 className="text-lg font-semibold">Current Group Members (Excluding Yourself):</h3>
                <ul className="list-disc pl-5 space-y-1">
                  {members.map((member, index) => (
                    <li key={index} className="text-sm text-muted-foreground">
                      {member.name} ({member.gender}, {member.age} yrs, from {member.country}, Phone: {member.phone})
                    </li>
                  ))}
                </ul>
              </div>
        ...
        {/* Create Group Button */}
        <div className="pt-6">
          <Button onClick={handleCreateSafariGroup} className="w-full h-12 text-lg">
