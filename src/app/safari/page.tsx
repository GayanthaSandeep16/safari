...
export default function SafariBookingPage() { // Renamed for clarity
  ...
  const availableSafarisForJoining = useQuery(api.queries.getSafarisForJoining, { status: 'active' }) || [];
  // Groups that the current user is a member of (for "Your Groups" section)
  const userGroups = useQuery(api.queries.getGroupsByUser, userId ? { userId } : 'skip') || [];
  ...
  const handleCreateIndividualSafari = async () => {
    if (!userId || !selectedDate || !title || !userCountry || !userGender || !userPhone) {
      toast.error('Required fields missing', {
        description: 'Please provide date, title, country, gender, and phone number.',
      });
      return;
    }
  ...
  const handleJoinPublicGroup = async () => {
    if (!selectedSafariToJoin || !userId || !userCountry || !userGender || !userPhone) {
      toast.error('Please select a safari, and provide your country, gender, and phone number.');
      return;
    }
    ...
    try {
      ...
      if (!groupToJoin) {
        toast.error('No open group found for this safari, or it is not shared.');
        return;
      }
      if (groupToJoin.status !== "open" || groupToJoin.currentSize >= groupToJoin.maxSize) {
          toast.error('This group is not available to join (full or closed).');
          return;
      }
      if (!groupToJoin.shareToken) {
          toast.error('This group does not have a share token and cannot be joined directly.');
          return;
      }

      // Create Payment for joining the group
      const paymentId = await createPayment({
        userId: userId as Id<'users'>,
        amount: groupToJoin.joinFee,
        currency: 'USD',
        method: 'Stripe (example)',
        status: 'completed',
        transactionId: nanoid(),
      ...
      // Join the group using its shareToken
      await joinGroup({
        shareToken: groupToJoin.shareToken, // Pass the share token
        userId: userId as Id<'users'>,
        paymentId,
      });
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Book an individual safari or join an existing group.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Plan a private safari just for yourself.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="indivBasePrice">Your Budget/Desired Price ($)</Label>
                <Input
                  id="indivBasePrice"
                  type="number"
                  value={basePrice}
              ...
              <div className="space-y-2">
                ...
                <Select value={userGender} onValueChange={(value: 'male' | 'female') => setUserGender(value)}>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Browse and join publicly available safari groups.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="joinSafariSelect">Select a Safari Group</Label>
                <Select value={selectedSafariToJoin} onValueChange={setSelectedSafariToJoin}>
                  <SelectTrigger id="joinSafariSelect">
                    <SelectValue placeholder="Choose a group to join" />
                  </SelectTrigger>
                  <SelectContent>
                    {availableSafarisForJoining.map((safari: Doc<'safaris'> & { group?: Doc<'groups'> }) =>
                      safari.group && safari.group.status === 'open' && safari.group.isShared ? (
                        <SelectItem key={safari._id} value={safari._id}>
                          {safari.title} ({format(new Date(safari.date), 'PPP')}) - {safari.group.currentSize}/{safari.group.maxSize}
                        </SelectItem>
                      ) : null
                    )}
                  </SelectContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Groups you are leading or have joined.</CardDescription>
          </CardHeader>
          <CardContent>
            {userGroups.length === 0 ? (
              <p className="text-muted-foreground text-center py-8">You are not part of any groups yet.</p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {userGroups.map((group: Doc<'groups'> & { safari?: Doc<'safaris'> }) => {
                    // Assuming getGroupsByUser query can fetch linked safari data
                    const safari = availableSafarisForJoining.find((s: Doc<'safaris'>) => s._id === group.safariId);
                    return (
                      <TableRow key={group._id}>
                        <TableCell>{safari?.title || 'Unknown'}</TableCell>
                        <TableCell>{safari?.date ? format(new Date(safari.date), 'PPP') : 'Unknown'}</TableCell>
                        <TableCell>
        ...
        <div className="flex justify-center mt-6">
          <Button onClick={() => router.push('/create-group')} className="w-fit px-8 py-3 text-lg">
        ...
        <div className="flex justify-center mt-4">
          <Button onClick={() => window.open('https://example-safari.com', '_blank')}>
            Visit Our Website
          </Button>
        </div>
      </div>
